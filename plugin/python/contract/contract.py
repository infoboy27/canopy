...
from .game import poker as gpoker
...
def key_for_poker_round(round_id: bytes) -> bytes:
    """State key for a poker round record. Capped at exactly 2 players
...
def poker_escrow_address(round_id: bytes) -> bytes:
    """Deterministic 20-byte account address holding a poker round's escrow.
    Distinct salt so it never collides with Bingo's/Roulette's/Domino's escrow
    for the same round_id."""
...
class Contract:
    ...
    def _check_message_open_poker(self, msg) -> PluginCheckResponse:
        if len(msg.operator_address) != 20:
            raise err_invalid_address()
        if not msg.round_id:
            raise PluginError(1, "plugin", "empty round_id")
        if len(msg.commitment) != 32:
            raise PluginError(1, "plugin", "commitment must be 32 bytes")
        if msg.small_blind == 0:
            raise err_invalid_amount()
        if msg.big_blind <= msg.small_blind:
            raise PluginError(1, "plugin", "big_blind must be greater than small_blind")
        if msg.buy_in == 0:
            raise err_invalid_amount()
        if msg.rake_bps > 10000:
            raise PluginError(1, "plugin", "rake_bps must be <= 10000")
    ...
    def _check_message_join_poker(self, msg) -> PluginCheckResponse:
        if len(msg.player_address) != 20:
            raise err_invalid_address()
        if not msg.round_id:
            raise PluginError(1, "plugin", "empty round_id")
    ...
    def _check_message_settle_poker(self, msg) -> PluginCheckResponse:
        if len(msg.operator_address) != 20:
            raise err_invalid_address()
        if not msg.round_id:
            raise PluginError(1, "plugin", "empty round_id")
        if not msg.seed:
            raise PluginError(1, "plugin", "empty seed")
        if not msg.actions:
            raise PluginError(1, "plugin", "empty action log")
        r = PluginCheckResponse()
        r.authorized_signers.append(msg.operator_address)
        return r

    def _check_message_expire_poker(self, msg) -> PluginCheckResponse:
        """Statelessly validate an 'expire_poker' message. Anyone may call
        this -- the deliver step enforces the round exists, is still open,
        and has passed its deadline."""
        if len(msg.caller_address) != 20:
            raise err_invalid_address()
    ...
    async def _deliver_message_open_poker(self, msg, height: int = 0) -> PluginDeliverResponse:
        ...
        rr.operator_address = msg.operator_address
        rr.commitment = msg.commitment
        rr.small_blind = msg.small_blind
        rr.big_blind = msg.big_blind
        rr.buy_in = msg.buy_in
        rr.rake_bps = msg.rake_bps
        rr.escrow_total = 0
        rr.status = 0
        rr.opened_height = height
    ...
    async def _deliver_message_join_poker(self, msg) -> PluginDeliverResponse:
        ...
        for r in resp.results:
            if r.query_id == qr:
                ...
                eb = r.entries[0].value if r.entries else None
        rr = unmarshal(PokerRound, rb) if rb else None
        if rr is None:
            raise PluginError(1, "plugin", "round not found")
        if rr.status != 0:
            raise PluginError(1, "plugin", "round not open")
        ...
        if bytes(msg.player_address) in [bytes(a) for a in rr.participant_addresses]:
            raise PluginError(1, "plugin", "address already joined")
        if msg.amount != rr.buy_in:
            raise PluginError(1, "plugin", "amount must equal the round's buy_in")
        player = unmarshal(Account, plb) if plb else Account()
        escrow = unmarshal(Account, eb) if eb else Account()
        if player.amount < msg.amount:
            raise err_insufficient_funds()
    ...
    async def _deliver_message_settle_poker(self, msg) -> PluginDeliverResponse:
        ...
        winner(s) and each participant's final stack itself -- an illegal
        action anywhere in the claimed log rejects the whole settle. Unlike
        ...
        rake = result.pot * rr.rake_bps // 10000
        net_pot = result.pot - rake
        per_winner = net_pot // len(result.winners)
        winner_payouts = [per_winner] * len(result.winners)
        winner_payouts[0] += net_pot - sum(winner_payouts)  # remainder to the first winner
        payout_by_winner_slot = dict(zip(result.winners, winner_payouts))

        final_payouts = {}
        for i, addr in enumerate(participants):
            share = payout_by_winner_slot.get(i, 0)
            final_payouts[addr] = result.stacks_remaining[i] + share
        ...
        for addr in participants:
            q = random.randint(0, 2**53)
            participant_qids.append((q, addr))
            keys.append(PluginKeyRead(query_id=q, key=key_for_account(addr)))
        resp = await self.plugin.state_read(self, PluginStateReadRequest(keys=keys))
        if resp.HasField("error"):
            out = PluginDeliverResponse(); out.error.CopyFrom(resp.error); return out
    ...
    async def _deliver_message_expire_poker(self, msg, height: int = 0) -> PluginDeliverResponse:
        """Refund every escrowed buy_in for a round the operator never
